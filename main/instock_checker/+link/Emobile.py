...
def scrape_and_save_to_excel(html_filename, excel_filename):
    ...
    for in_stock_tag in in_stock_tags:
        ...
        if caption_div:
            product_name_tag = caption_div.find("a")
            if product_name_tag:
                product_name = product_name_tag.text.strip()
                product_link = product_name_tag['href'].split('?')[0]  # Remove the ?limit=1000 part
                price = caption_div.find("p", class_="price").text.strip()
                products.append(product_name)
                product_links.append(product_link)  # Append the product link
                prices.append(price)

    data = {"Product Name": products, "Product Link": product_links, "Price": prices}  # Add product links to data
